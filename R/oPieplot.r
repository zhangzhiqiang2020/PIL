...
#'
#' \code{oPieplot} is supposed to visualise data frame using pie plots. It returns an object of class "ggplot".
#'
...
#' }
oPieplot <- function(df, columns, colormap = "ggplot2", pie.radius = NULL, pie.color = "transparent", pie.color.alpha = 1, pie.thick = 0.1, legend.title = "", gp = NULL) {
  if (!is(df, "data.frame")) {
  ...
  my_colors <- oColormap(colormap)(length(columns))
