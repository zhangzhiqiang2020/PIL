...
oHeatmap <- function(data, reorder = c("none", "row", "col", "both"), colormap = "spectral", ncolors = 64, zlim = NULL, barwidth = 0.3, barheight = NULL, nbin = 64, legend.title = "", x.rotate = 45, x.text.size = 6, x.text.hjust = 0, y.text.size = 6, legend.text.size = 4, legend.title.size = 6, shape = 19, size = 2, plot.margin = unit(c(5.5, 5.5, 5.5, 5.5), "pt"), font.family = "sans", na.color = "transparent", data.label = NULL, label.size = 1, label.color = "black", ...) {
  ...
  if (nrow(mat_val) > 1 & nrow(mat_val) > 1) {
    ...
    if (ncol(mat_val) > 1 & (reorder == "row" | reorder == "both")) {
      ...
      ####
      set.seed(825)
      tree_bs <- supraHex::visTreeBootstrap(mat, visTree = FALSE, ...)
    ...
    if (nrow(mat_val) > 1 & (reorder == "col" | reorder == "both")) {
      ...
      ####
      set.seed(825)
      tree_bs <- supraHex::visTreeBootstrap(t(mat), visTree = FALSE, ...)
  ...
  if (is(mat_val, "data.frame")) {
    ...
    df <- mat_val %>%
      dplyr::mutate(gene = rownames(mat_val)) %>%
    ...
    if (shape == "tile") {
      ...
      if (!flag_factor) {
        gp <- gp + scale_fill_gradientn(colors = oColormap(colormap)(ncolors), limits = zlim, guide = guide_colorbar(title = legend.title, title.position = "top", barwidth = barwidth, barheight = barheight, nbin = nbin, draw.ulim = FALSE, draw.llim = FALSE), na.value = na.color)
      } else {
        gp <- gp + scale_fill_manual(legend.title, values = oColormap(colormap)(length(lvl)), labels = lvl)
      ...
      if (!flag_factor) {
        gp <- gp + scale_colour_gradientn(colors = oColormap(colormap)(ncolors), limits = zlim, guide = guide_colorbar(title = legend.title, title.position = "top", barwidth = barwidth, barheight = barheight, nbin = nbin, draw.ulim = FALSE, draw.llim = FALSE), na.value = na.color)
      } else {
        gp <- gp + scale_colour_manual(legend.title, values = oColormap(colormap)(length(lvl)), labels = lvl)
        if (is.null(barheight)) {
          barheight <- barwidth
        }
        gp <- gp + theme(legend.key.width = unit(barwidth, "pt"), legend.key.height = unit(barheight, "pt")) + guides(col = guide_legend(ncol = 1))
