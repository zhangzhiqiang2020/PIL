...
oSEAforest <- function(obj, top = 10, adjp.cutoff = 0.05, colormap = "brewer.Reds", zlim = NULL, color.title = expression(-log[10]("adjp")), shape = 18, size.range = c(0, 1), slim = NULL, size.title = "Overlap", wrap.width = NULL, legend.direction = c("auto", "horizontal", "vertical"), sortBy = c("or", "none")) {
  ...
  if (is(top, "numeric")) {
    top <- as.integer(top)
    df %>%
      dplyr::group_by(group, namespace) %>%
      dplyr::top_n(top, or) %>%
      dplyr::ungroup() %>%
      dplyr::select(name) -> df_name
  ...
  if (sortBy == "or") {
    df %>%
      dplyr::filter(adjp < adjp.cutoff) %>%
      dplyr::arrange(group, onto, namespace, or, name) %>%
      dplyr::mutate(name = forcats::fct_inorder(name)) -> df
  } else {
    df %>%
      dplyr::filter(adjp < adjp.cutoff) %>%
      dplyr::arrange(group, onto, namespace, rev(name)) %>%
      dplyr::mutate(name = forcats::fct_inorder(name)) -> df
  }
  ...
  df %>% ggplot(aes(y = name, x = log2(or), xmin = log2(CIl), xmax = log2(CIu), color = color)) +
    geom_pointrange(aes(size = size), shape = shape) -> gp
