#' Function to conduct set enrichment analysis
#'
#' \code{oSEA} is supposed to conduct set enrichment analysis (SEA) given the input data, annotation and/or ontology. The annotation is provided as an object of class "SET", while the ontology as an object of class "igraph", usually direct acyclic graph (DAG). It returns an object of class "eSET". Enrichment analysis is based on either Fisher's exact test or Hypergeometric test. The test can respect the hierarchy of the ontology.
#'
#' @param data an input vector containing genes (or SNPs)
#' @param set an object of class 'SET' to represent annotations or a list each containg annotations
#' @param ig an object of class "igraph" to represent DAG. If provided, it must have node/vertice attributes: "name" (i.e. "Term Name"), "id" (i.e. "Term ID"), "namespace" (i.e "Term Namespace") and "distance" (i.e. Term Distance: the distance to the root; always 0 for the root itself). By default it is NULL
#' @param background a background vector. It contains genes as the test background. If NULL, by default all annotatable are used as background
...
#' \itemize{
#'  \item{\code{data}: a tibble containing a column 'data' for input data analysed}
#'  \item{\code{background}: a tibble containing a column 'background' for the background used}
#'  \item{\code{ig}: NULL or an "igraph" object}
#'  \item{\code{info}: a tibble of nSet X 14 containing set enrichment information, where nSet is the number of sets, and the 14 columns including "id" (set ID), "name" (set name), "nA" (number of annotations), "nO" (number of overlaps), "fc" (fold change), "zscore" (z-score), "pvalue" (p-value), "adjp" (adjusted p-value), "or" (odds ratio), "CIl" (lower bound confidence interval for the odds ratio), "CIu" (upper bound confidence interval for the odds ratio), "distance" (distance to the root), "namespace", "overlap" (the detailed members for overlaps)}
#' }
...
#' set2 <- set
#' set2$info <- set2$info %>% filter(!str_detect(namespace, "L1000"))
#'
#' eset <- oSEA(data, set, ig)
#' eset$ig
#' eset %>%
#'   oSEAextract() %>%
#'   write_delim("results_eset.txt", "\t")
#' eset %>%
#'   oSEAextract() %>%
#'   filter(distance == "DiseaseDrug") %>%
#'   oSEAforest(top = 2)
#' eset %>%
#'   oSEAextract() %>%
#'   filter(adjp < 0.01) %>%
...
#'   top_n(2, -adjp) %>%
#'   ungroup() %>%
#'   oSEAwheel(ig, fixed = F, edge.color = "namespace", leaf.label.orientation = "outwards", leaf.label.expansion = 1.15, limit.expansion = 2, leaf.label.size = 1.5)
#'
#' # ggraph
#' ## 1) based on eset$ig (default)
#' gp <- oSEAggraph(eset, fixed = F, leave = F, slim = c(0, 10), node.label.direction = "leftright")
#' ## 2) based on eset$ig (shortest version)
#' subg <- oDAGinduce(eset$ig, path.mode = "shortest_paths")
#' gp <- oSEAggraph(eset, subg, fixed = F, leave = F, slim = c(0, 10), node.label.direction = "leftright")
...
#' gp <- oSEAggraph(eset, clade, fixed = F, leave = F, slim = c(0, 10), node.label.direction = "leftright")
#' ## 4) do comparisons
#' gp <- oSEAggraph(eset, fixed = F, slim = c(0, 10), node.label.size = 0)
#' gp1 <- oSEAggraph(eset, subg, fixed = F, slim = c(0, 10), node.label.size = 0)
#' gp2 <- oSEAggraph(eset, clade, fixed = F, slim = c(0, 10), node.label.size = 0)
#' library(patchwork)
#' gp + gp1 + gp2 + plot_layout(guides = "collect")
#'
#' # advanced visual
#' gp <- oSEAggraph(df, fixed = F, leave = F, layout = "dendrogram", node.label.size = 0)
#' ## alternative
#' eset$info %>% filter(adjp < 0.05) -> df
#' eset$ig %>% oDAGtree() -> ig.tree
#' gp <- oSEAggraph(df, ig.tree, fixed = F, leave = F, layout = "dendrogram", node.label.size = 0)
#' ## labelling
#' gp + ggraph::geom_node_text(aes(x = x * 1.1, y = y * 1.1, filter = leaf, label = name, angle = node_angle(x, y)), repel = F, hjust = 0, size = 2) + expand_limits(x = c(-2, 2), y = c(-2, 2)) -> gp
#' gp + geom_edge_diagonal2(aes(color = node.term_namespace, alpha = stat(index)), width = 0.2) + guides(edge_alpha = F) -> gp1
...
oSEA <- function(data, set, ig = NULL, background = NULL, size.range = c(10, 2000), min.overlap = 5, which.distance = NULL, test = c("fisher", "hypergeo", "binomial"), background.annotatable.only = NULL, p.tail = c("one-tail", "two-tails"), p.adjust.method = c("BH", "BY", "bonferroni", "holm", "hochberg", "hommel"), ontology.algorithm = c("none", "pc", "elim", "lea"), elim.pvalue = 1e-2, lea.depth = 2, path.mode = c("all_paths", "shortest_paths", "all_shortest_paths"), true.path.rule = FALSE, verbose = TRUE) {
  ...
  if (is(set, "SET")) {
    name <- member <- NULL
    anno <- set$info %>%
      dplyr::mutate(member = purrr::map(member, ~ sort(.x))) %>%
      dplyr::select(name, member) %>%
      tibble::deframe()
  } else {
    if (is.list(set)) {
      if (is.null(names(set))) {
        names(set) <- paste("C", 1:length(set), sep = "")
      }
      #######
      anno <- set[lapply(set, length) > 0]
      if (length(anno) == 0) {
        return(NULL)
      }
    } else {
      stop("There is no input for the object SET.\n")
    }
  }

  if (!is(ig, "igraph")) {
    # warnings("The input 'ig' must be 'igraph' object.\n")
    # return(NULL)

    ## 'ontology.algorithm' always 'none' if 'ig' is not provided
    ...
    subg <- oDAGanno(ig = ig, anno = anno, path.mode = path.mode, true.path.rule = true.path.rule, verbose = verbose)

    gs <- V(subg)$anno
    names(gs) <- V(subg)$name

    gs.distance <- V(subg)$distance
  ...
  id <- name <- distance <- namespace <- NULL
  if (is(subg, "igraph")) {
    subg <- oDAGinduce(subg, nodes_query = terms, path.mode = path.mode)
    set_info <- subg %>%
      oIG2TB("nodes") %>%
      dplyr::transmute(name = name, id = id, distance = distance, namespace = namespace)
  } else {
    if (is(set, "SET")) {
      id <- name <- distance <- namespace <- NULL
      set_info <- set$info %>%
        dplyr::semi_join(tibble::tibble(name = terms), by = "name") %>%
        dplyr::select(name, id, distance, namespace)
    } else {
      set_info <- tibble::tibble(name = names(anno), id = names(anno), distance = 1, namespace = "Customised")
    }
  }
      ...
      for (i in nLevels:1) {
        ...
        for (currNode in currNodes) {
          ...
          if (pvalue < pval.cutoff) {
            ...
            dag.ancestors <- oDAGinduce(subg, currNode, path.mode = "all_paths")
  ...
  ## update set_info
  ind <- match(set_info$name, names(pvals))
  ...
  id <- name <- nA <- nO <- fc <- zscore <- pvalue <- adjp <- or <- distance <- namespace <- overlap <- anno <- NULL

  info <- dplyr::bind_cols(set_info, tibble::tibble(fc = fcs, zscore = zscores, pvalue = pvals, adjp = adjpvals, or = ors, CIl = CIl, CIu = CIu, overlap = overlaps, anno = gs)) %>%
    dplyr::mutate(nA = purrr::map_int(anno, ~ length(.x)), nO = purrr::map_int(overlap, ~ length(.x))) %>%
    dplyr::select(name, id, nA, nO, fc, zscore, pvalue, adjp, or, CIl, CIu, distance, namespace, overlap) %>%
    dplyr::arrange(pvalue, -or)

  data <- tibble::tibble(data = genes.group) %>% dplyr::arrange(data)
  background <- tibble::tibble(background = genes.universe) %>% dplyr::arrange(background)

  eSET <- list(data = data, background = background, ig = subg, info = info, set_info = set_info)
  class(eSET) <- "eSET"
