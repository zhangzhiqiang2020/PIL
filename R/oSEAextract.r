...
oSEAextract <- function(obj, sortBy = c("adjp", "pvalue", "zscore", "fc", "nA", "nO", "or", "id", "name", "distance")) {
  ...
  if (is(obj, "eSET") | is(obj, "eSAD")) {
    ...
    if (is(obj, "eSET")) {
      ...
      obj %>%
        dplyr::select(group, onto, info) %>%
        tidyr::unnest(info) %>%
        mutate(overlap = purrr::map_chr(overlap, ~ str_c(.x, collapse = ", "))) -> tab
    }

    switch(sortBy,
      adjp = {
        res <- tab %>% dplyr::arrange(pvalue, -zscore)
      },
      pvalue = {
        res <- tab %>% dplyr::arrange(pvalue, -zscore)
      },
      zscore = {
        res <- tab %>% dplyr::arrange(-zscore, pvalue)
      },
      fc = {
        res <- tab %>% dplyr::arrange(-fc, pvalue)
      },
      or = {
        res <- tab %>% dplyr::arrange(-or, pvalue)
      },
      nA = {
        res <- tab %>% dplyr::arrange(-nA, pvalue, -zscore)
      },
      nO = {
        res <- tab %>% dplyr::arrange(-nO, pvalue, -zscore)
      },
      name = {
        res <- tab %>% dplyr::arrange(name)
      },
      id = {
        res <- tab %>% dplyr::arrange(id)
      },
      distance = {
        res <- tab %>% dplyr::arrange(distance, pvalue, -zscore)
      }
    )
